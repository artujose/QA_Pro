...
public class RegisterMember extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request,
		...
		try {
			...
			// " ms elapsed for creating db pool");

			String userNameCountSql = "select count(user_name) from user where user_name = ?";

			userNameCountStatement = connection
					.prepareStatement(userNameCountSql);
			...
			ResultSet autoIncrementResultSet = preparedStatement
					.getGeneratedKeys();
			...
			String emailValidationInsertSql = "INSERT INTO qapro.email_validation"
					...
					+ " VALUES	(?,?,?,?,?,?)";
			PreparedStatement validationStatement = connection
					.prepareStatement(emailValidationInsertSql);
			...
			validationStatement.setTimestamp(5,
					new Timestamp(new Date().getTime()));
			...
			System.out.println(request.getScheme() + "://"
					...
					+ request.getContextPath() + "/validate?key=" + key
					+ "&userid=" + autoIncrementedUserId);

			// lastTime = System.currentTimeMillis();
			// System.out.println(lastTime -
			// currentTime+" ms elapsed for inserting");
