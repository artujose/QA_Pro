...
import com.qapro.util.SqlBindingUtil;
import com.qapro.util.SqlParameterValues;
import com.qapro.util.SqlRowMapper;

public abstract class JavathlonJdbcTemplate<T> {

	protected List<T> getList(String sql, SqlParameterValues values,
		...
		try {
			...
			HashMap<String, Integer> sqlBindings = (HashMap<String, Integer>) SqlBindingUtil
			...
			putBindingsToPreparedStatement(preparedStatement, values,
					sqlBindings);
	...
	private void putBindingsToPreparedStatement(PreparedStatement ps,
			SqlParameterValues values, HashMap<String, Integer> sqlBindings)
			throws Exception {
		for (Entry<String, Object> entry : values.getValues().entrySet()) {
			...
			if (bindingValue instanceof String)
				...
				ps.setDate(bindingOrder, new java.sql.Date(
						((Date) bindingValue).getTime()));
		}
	}

	public T queryForObject(String sql, SqlParameterValues values,
			SqlRowMapper<T> mapper) throws Exception {
		...
		return objectList != null && objectList.size() > 0 ? objectList.get(0)
				: null;
	...
	protected Long insertItem(String sql, SqlParameterValues values)
			throws Exception {
	...
	protected T getObject(String sql, SqlRowMapper<T> mapper,
			ResultSet resultSet) {
	...
	protected List<T> convertResultSetToObjectList(SqlRowMapper<T> mapper,
			ResultSet resultSet) {
