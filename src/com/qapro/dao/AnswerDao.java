...
public class AnswerDao extends JavathlonJdbcTemplate<Answer> {
	...
	public List<Answer> getAnswersToQuestion(Long questionId) {
		...
		return this.getList(sql,
				new SqlParameterValues().addValue("question_id", questionId),
				new AnswerRowMapper());
	}
	...
	public Answer getAnswerById(Long id) {
		...
		List<Answer> answerList = this.getList(sql,
				new SqlParameterValues().addValue("id", id),
				new AnswerRowMapper());
	...
	public static void main(String args[]) {
		...
		try {
			 ...
			 * answer.setText("Of course Javathlon"); answer.setQuestionId(4L);
			 * answer.setResponderId(1L); answerDao.insertAnswer(answer);
			 */
