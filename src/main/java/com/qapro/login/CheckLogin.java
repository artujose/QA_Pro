		...
		try {
			...
			if (rs.next()) {
				...
				if (isActive == false) {
					...
					PreparedStatement priviligesStatement = con
							.prepareStatement(getPermissionSql);

					priviligesStatement.setString(1, role_name);

					ResultSet priviligesResult = priviligesStatement
							.executeQuery();

					ArrayList<String> permissionList = new ArrayList<String>();
					while (priviligesResult.next()) {
						String priviligeName = priviligesResult.getString(1);
						boolean isActivePermission = priviligesResult
								.getBoolean(2);
